...
from utils.config_loader import load as load_config_overrides
from services.wifi import WiFiService
from services.weather_service import WeatherService
from services.web_config import WebConfig
...
def main():
    ...
    while True:
        ...
        if weather.get("ok", False):
            if not within_hours(weather.get("time_rtc"), config.SLEEP_START, config.SLEEP_END):
                t0 = time.ticks_ms()
                ...
                deadline  = time.ticks_add(time.ticks_ms(), remaining * 60 * 1000)
            ...
            if not reconnect:
                ...
                retry_at = time.ticks_add(time.ticks_ms(), 30_000)
                while time.ticks_diff(retry_at, time.ticks_ms()) > 0:
                    web_config.poll()
                    time.sleep_ms(500)
            else:
